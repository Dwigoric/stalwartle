...
module.exports = class extends Argument {
    ...
    async run(arg, possible, msg) {
        if (!msg.guild) throw `${this.client.constants.EMOTES.xmark}  ::  There must be a server to get the member from.`;
        ...
        switch (querySearch.length) {
            case 0: throw `${this.client.constants.EMOTES.xmark}  ::  \`${possible.name}\` must be a valid user tag, ID, mention, or nickname from this server.`;
