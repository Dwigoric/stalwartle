...
module.exports = class extends Event {
	...
	async run(oldState, newState) {
		...
		const queue = this.client.gateways.music.get(newState.guild.id, true).get('queue');
		...
		return this.client.setTimeout(guild => {
			...
			if (queue[0].requester === this.client.user.id) this.client.gateways.music.get(newState.guild.id, true).update('queue', []);
			return null;
