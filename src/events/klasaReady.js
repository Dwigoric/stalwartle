...
module.exports = class extends Event {
	...
	async run() {
		...
		if (this.client.application.botPublic) this.client.setGuildCount().then(() => this.client.setInterval(() => this.client.setGuildCount(), 1000 * 60 * 5));
		...
		this.client.channels.get(channel).send({
			embed: new MessageEmbed()
				...
				.setDescription(`**Creeping through Discord...**\nand doing some magic!\n\nCurrently running on **${this.client.guilds.size}** guilds with **${await this.client.userCount()}** users.`) // eslint-disable-line max-len
