    ...
    async run(guild) {
        const message = new MessageEmbed()
            ...
            .setDescription([
                ...
                `\nTo get started, please use \`${this.container.stores.get('gateways').guilds.get(guild.id).prefix}help\` here or on any text channel. You will be given a list of commands.`,
                `Please feel free to look at the command list. If you want me to serve more Discord users, just use the \`${this.container.stores.get('gateways').guilds.get(guild.id).prefix}invite\` command!`,
                '\nI can play music, moderate users, search lyrics, search Steam, search a lot more other stuff, and more!',
                `Most users use the music feature. Run \`${this.container.stores.get('gateways').guilds.get(guild.id).prefix}help music\` and \`${this.container.stores.get('gateways').guilds.get(guild.id).prefix}help play\` for more information!`,
