...
module.exports = class extends Listener {
    ...
    async run(msg) {
        ...
        if (this.container.client.gateways.afk.get(msg.author.id).timestamp && !this.container.client.gateways.users.get(msg.author.id).afktoggle) {
            const wbMsg = `${this.container.client.constants.EMOTES.blobwave}  ::  Welcome back, **${msg.author}**! I've removed your AFK status.`;
        ...
        msg.send([
            `${this.container.client.constants.EMOTES.blobping}  ::  ${msg.author}, **${await msg.guild.members.fetch(afkUser.id).then(mb => mb.displayName).catch(() => afkUser.username)}** is currently AFK. [Last seen ${Duration.toNow(timestamp)} ago]`, // eslint-disable-line max-len
