...
module.exports = class extends Listener {
    ...
    async giveRole(member, type) {
        ...
        if (!role) {
            member.guild.owner.user.send(`The role **${this.container.stores.get('gateways').get('guildGateway').get(member.guild.id).autorole[type]}** doesn't exist anymore. Autorole aborted.`).catch(() => null);
