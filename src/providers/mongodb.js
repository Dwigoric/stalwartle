...
module.exports = class extends Provider {
    ...
    async init() {
        const connection = mergeDefault({
            host: 'localhost',
            port: 27017,
            db: this.container.client.options.mongodbName,
            options: {}
        }, this.container.client.options.providers.mongodb);

        // If full connection string is provided, use that, otherwise fall back to individual parameters
        const connectionString = this.container.client.options.providers.mongodb.connectionString || `mongodb://${connection.user}:${connection.password}@${connection.host}:${connection.port}/${connection.db}`;
