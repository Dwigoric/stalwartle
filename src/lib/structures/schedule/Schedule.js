...
class Schedule {
    ...
    async create(taskName, time, options) {
        ...
        schedules.push(task);
        await this.client.stores.get('gateways').get('clientGateway').update(this.client.user.id, { schedules });
        return task;
    }
    ...
    async delete(id) {
        ...
        if (task) await this.client.stores.get('gateways').get('clientGateway').update(this.client.user.id, { schedules });

        return this;
    }

    async clear() {
        this.client.stores.get('gateways').get('clientGateway').update(this.client.user.id, { schedules: this.client.stores.get('gateways').get('clientGateway').defaults.schedules });
        this.tasks = [];
    }
