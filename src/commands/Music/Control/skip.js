...
module.exports = class extends Command {
    ...
    async messageRun(msg, [entry]) {
        ...
        if (msg.flagArgs.force && await msg.hasAtLeastPermissionLevel(5)) {
            ...
            return msg.send(`${this.container.constants.EMOTES.tick}  ::  Successfully forcibly skipped the music for this server.`);
        }
        if (this.container.client.cache.guilds.get(msg.guild.id).voteskips.includes(msg.author.id)) throw `${this.container.constants.EMOTES.xmark}  ::  You've already voted to skip the current song.`;
        const { members } = msg.guild.me.voice.channel;
        if (!members.has(msg.author.id)) throw `${this.container.constants.EMOTES.xmark}  ::  You are not connected to the voice channel I'm playing on.`;
        ...
        msg.guild.clearVoteskips();
        this.container.lavacord.players.get(msg.guild.id).stop();
        return msg.send(`${this.container.constants.EMOTES.tick}  ::  Successfully skipped the music for this server.`);
    }

    async skipToEntry(msg, entry) {
        const { queue = [] } = await this.container.databases.default.get('music', msg.guild.id) || {};
        if (queue.length < 2) throw `${this.container.constants.EMOTES.xmark}  ::  There is no queue entry to skip to.`;
        if (entry > queue.length - 1) throw `${this.container.constants.EMOTES.xmark}  ::  The server queue only has ${queue.length - 1} entr${queue.length - 1 === 1 ? 'y' : 'ies'}.`;
        ...
        return msg.send(`${this.container.constants.EMOTES.tick}  ::  Successfully skipped to entry \`#${entry}\`.`);
    }
