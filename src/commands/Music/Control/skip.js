...
module.exports = class extends Command {
    ...
    async run(msg, [entry]) {
        ...
        if (this.client.cache.guilds.get(msg.guild.id).voteskips.includes(msg.author.id)) throw `${this.client.constants.EMOTES.xmark}  ::  You've already voted to skip the current song.`;
        ...
        if (this.client.cache.guilds.get(msg.guild.id).voteskips.length <= requiredVotes) {
            return msg.send([
                `${this.client.constants.EMOTES.tick}  ::  Successfully added your vote to skip the current song!`,
                `Current votes: \`${this.client.cache.guilds.get(msg.guild.id).voteskips.length}\`.`,
