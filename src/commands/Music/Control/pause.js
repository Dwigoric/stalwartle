...
module.exports = class extends Command {
    ...
    async messageRun(msg) {
        ...
        if (this.container.erela.get(msg.guild.id).paused) return reply(msg, `${this.container.constants.EMOTES.xmark}  ::  Music is already paused! Resume it with \`${this.container.stores.get('gateways').get('guildGateway').get(msg.guild.id, 'prefix')}resume\`.`); // eslint-disable-line max-len

        this.container.erela.get(msg.guild.id).pause(true);
