		...
		do {
			...
			choice = await msg.prompt([
				...
				finds.map((result, index) => {
					...
					return `\`${index + 1}\`. [**${escapeMarkdown(result.info.title)}** by ${escapeMarkdown(result.info.author)}](${result.info.uri}) \`${new Timestamp(`${length >= 86400000 ? 'DD:' : ''}${length >= 3600000 ? 'HH:' : ''}mm:ss`).display(length)}\``; // eslint-disable-line max-len
				}).join('\n')
