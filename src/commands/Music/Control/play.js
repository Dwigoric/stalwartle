		...
		if (msg.flagArgs.force && await msg.hasAtLeastPermissionLevel(5)) {
			...
			if (!msg.channel.permissionsFor(this.client.user).has('EMBED_LINKS')) {
				...
				msg.send(queue.length >= 2 && (!msg.guild.player || (msg.guild.player && !msg.guild.player.playing)) ?
					// eslint-disable-next-line max-len
					`There are songs in your queue from your previous session! You can run \`${msg.guild.prefix}remove 1${queue.length >= 3 ? `-${queue.length - 1}` : ''}\` then \`${msg.guild.prefix}skip\` to start over.` :
