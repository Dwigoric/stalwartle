...
module.exports = class extends Command {
    ...
    constructor(...args) {
        ...
        this
            .createCustomResolver('string', (arg, possible, msg, [action]) => {
                ...
                if (!arg) {
                    if (action === 'add') throw `${this.container.client.constants.EMOTES.xmark}  ::  Please provide the URL of the song(s) you want to add to the playlist.`;
                    else throw `${this.container.client.constants.EMOTES.xmark}  ::  Please provide the range of items to remove from the playlist.`;
                }
    ...
    async messageRun(msg) {
        ...
        return display
            ...
            .run(await message.edit(`${this.container.client.constants.EMOTES.tick}  ::  Server music playlist has been loaded!`), { filter: (reaction, author) => author === msg.author });
    }
    ...
    async add(msg, [songs]) {
        ...
        if (!await msg.hasAtLeastPermissionLevel(5)) throw `${this.container.client.constants.EMOTES.xmark}  ::  Only DJs can configure the playlist!`;
        if (!URL_REGEX.test(songs) && !['.m3u', '.pls'].includes(songs.slice(-4))) throw `${this.container.client.constants.EMOTES.xmark}  ::  Unsupported URL.`;

        if (['queue', 'queuereplace'].includes(songs)) {
            switch (songs) {
                case 'queue':
                    this.addToPlaylist(msg, (await this.container.client.providers.default.get('music', msg.guild.id) || {}).queue || []);
                    break;
                case 'queuereplace':
                    this.container.client.providers.default.update('music', msg.guild.id, { playlist: (await this.container.client.providers.default.get('music', msg.guild.id) || {}).queue || [] }, true);
                    break;
            }
            return null;
        }
        ...
        if (loadType === 'LOAD_FAILED') throw `${this.container.client.constants.EMOTES.xmark}  ::  Something went wrong when loading your tracks. Sorry 'bout that! Please try again.`;
        if (loadType === 'NO_MATCHES') throw `${this.container.client.constants.EMOTES.xmark}  ::  You provided an invalid stream or URL.`;
    ...
    async remove(msg, [items]) {
        ...
        if (isNaN(items[0])) throw `${this.container.client.constants.EMOTES.xmark}  ::  Invalid playlist entry given. Refer to \`${msg.guild.settings.get('prefix')}help remove\` for more information.`;
        if (!items[1]) items = items[0] - 1; // eslint-disable-line prefer-destructuring
        else items = [items[0] - 1, items[1] - 1];
        if (items === -1 || items[0] === -1) throw `${this.container.client.constants.EMOTES.xmark}  ::  All lists start at 1...`;
        const { playlist = [] } = await this.container.client.providers.default.get('music', msg.guild.id) || {};
        if (!playlist.length) throw `${this.container.client.constants.EMOTES.xmark}  ::  There are no items in the playlist. Add one using \`${msg.guild.settings.get('prefix')}play\``;
        if (Array.isArray(items)) {
            if (items[0] > items[1]) throw `${this.container.client.constants.EMOTES.xmark}  ::  Invalid playlist range. The first number must be less than the second.`;
            if (items[0] > playlist.length - 1 || items[1] > playlist.length - 1) throw `${this.container.client.constants.EMOTES.xmark}  ::  There are only ${playlist.length - 1} items in the playlist.`;
            playlist.splice(items[0], items[1] - items[0] + 1);
            msg.send(`${this.container.client.constants.EMOTES.tick}  ::  Successfully removed items \`#${items[0] + 1}\` to \`#${items[1] + 1}\` from the playlist.`);
        } else {
            if (items > playlist.length - 1) throw `${this.container.client.constants.EMOTES.xmark}  ::  There are only ${playlist.length - 1} items in the playlist.`;
            playlist.splice(items, 1);
            msg.send(`${this.container.client.constants.EMOTES.tick}  ::  Successfully removed song \`#${items + 1}\` from the playlist.`);
        }
        return this.container.client.providers.default.update('music', msg.guild.id, { playlist });
    }

    async clear(msg) {
        if (!await msg.hasAtLeastPermissionLevel(5)) throw `${this.container.client.constants.EMOTES.xmark}  ::  Only DJs can configure the playlist!`;
        this.container.client.providers.default.update('music', msg.guild.id, { playlist: [] });
        msg.send(`${this.container.client.constants.EMOTES.tick}  ::  Successfully cleared the music playlist for this server.`);
    }

    async export(msg) {
        const { playlist = [] } = await this.container.client.providers.default.get('music', msg.guild.id) || {};
        if (!playlist.length) throw `${this.container.client.constants.EMOTES.xmark}  ::  The playlist is empty. Add one using the \`${msg.guild.settings.get('prefix')}playlist add\` command.`;
        let choice;
        do {
        ...
        switch (choice.content) {
            case 'file': {
                ...
                return msg.channel.sendFile(Buffer.from(playlist.map(track => track.info.uri).join('\r\n')), 'output.txt', `${this.container.client.constants.EMOTES.tick}  ::  Exported the playlist as file.`); // eslint-disable-line max-len
            }
            case 'haste':
            case 'hastebin': {
                ...
                return msg.send(`${this.container.client.constants.EMOTES.tick}  ::  Exported the playlist to hastebin: <https://hastebin.com/${key}.stalwartle>`);
            }
        }
        return null;
    }
    ...
    async move(msg) {
        if (!await msg.hasAtLeastPermissionLevel(5)) throw `${this.container.client.constants.EMOTES.xmark}  ::  Only DJs can configure the playlist!`;
        const { playlist = [] } = await this.container.client.providers.default.get('music', msg.guild.id) || {};
        if (playlist.length < 2) throw `${this.container.client.constants.EMOTES.xmark}  ::  There is no playlist item to move.`;
        let entry;
        do entry = await msg.prompt(`${this.container.client.constants.EMOTES.loading}  ::  Which playlist item do you want to move? Reply with its playlist number.`);
        while (!parseInt(entry));
        entry = parseInt(entry) - 1;
        let position;
        do position = await msg.prompt(`${this.container.client.constants.EMOTES.loading}  ::  To which position do you want \`#${entry + 1}\` to be moved to? Reply with the position number.`);
        while (!parseInt(position));
        position = parseInt(position) - 1;
        if (entry > playlist.length - 1 || position > playlist.length - 1) throw `${this.container.client.constants.EMOTES.xmark}  ::  The playlist only has ${playlist.length - 1} entr${playlist.length - 1 === 1 ? 'y' : 'ies'}.`; // eslint-disable-line max-len
        if (entry === position) throw `${this.container.client.constants.EMOTES.xmark}  ::  What's the point of moving a playlist to the same position?`;
        playlist.splice(position, 0, playlist.splice(entry, 1)[0]);
        await this.container.client.providers.default.update('music', msg.guild.id, { playlist });
        return msg.send(`${this.container.client.constants.EMOTES.tick}  ::  Successfully moved item \`#${entry + 1}\` to position \`#${position + 1}\`.`);
    }

    async shuffle(msg) {
        if (!await msg.hasAtLeastPermissionLevel(5)) throw `${this.container.client.constants.EMOTES.xmark}  ::  Only DJs can configure the playlist!`;
        const { playlist = [] } = await this.container.client.providers.default.get('music', msg.guild.id) || {};
        if (!playlist.length) throw `${this.container.client.constants.EMOTES.xmark}  ::  There are no songs in the playlist. Add one with \`${msg.guild.settings.get('prefix')}playlist add\``;
        if (playlist.length === 1) throw `${this.container.client.constants.EMOTES.xmark}  ::  There is only one playlist item... I have nothing to shuffle!`;
        ...
        msg.send(`${this.container.client.constants.EMOTES.tick}  ::  Successfully shuffled the playlist. Check it out with \`${msg.guild.settings.get('prefix')}playlist\``);
    }
    ...
    async addToPlaylist(msg, items) {
        ...
        if (Array.isArray(items)) {
            ...
            if (playlist.length >= msg.guild.settings.get('music.maxPlaylist')) throw `${this.container.client.constants.EMOTES.xmark}  ::  The music playlist for **${msg.guild.name}** has reached the limit of ${msg.guild.settings.get('music.maxPlaylist')} songs; currently ${playlist.length}. Change limit via \`${msg.guild.settings.get('prefix')}conf set music.maxPlaylist <new limit>\`.`; // eslint-disable-line max-len
        ...
        await this.container.client.providers.default.update('music', msg.guild.id, { playlist }, true);
        return playlist;
    }
