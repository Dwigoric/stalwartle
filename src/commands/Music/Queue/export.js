...
module.exports = class extends Command {
    ...
    async messageRun(msg) {
        ...
        if (!queue.length) throw `${this.container.client.constants.EMOTES.xmark}  ::  The queue is empty. Add one using the \`${msg.guild.settings.get('prefix')}play\` command.`;
        let choice;
        do {
        ...
        switch (choice.content) {
            case 'file': {
                ...
                return msg.channel.sendFile(Buffer.from(queue.map(track => track.info.uri).join('\r\n')), 'output.txt', `${this.container.client.constants.EMOTES.tick}  ::  Exported the queue as file.`); // eslint-disable-line max-len
            }
            case 'haste':
            case 'hastebin': {
                ...
                return msg.send(`${this.container.client.constants.EMOTES.tick}  ::  Exported the queue to hastebin: <https://hastebin.com/${key}.stalwartle>`);
            }
        }
        return null;
    }
