...
module.exports = class extends Command {
	...
	async run(msg, [member, duration = Infinity, ...reason], force) {
		if (!msg.guild.settings.muteRole) throw '<:crossmark:508590460688924693>  ::  The mute role has not yet been set up for this server. You can do so by using the `s.muterole` command.';
		...
		if (member && !force) {
			if (member.permissions.bitfield >= msg.member.permissions.bitfield) throw '<:crossmark:508590460688924693>  ::  You cannot mute this user.';
			if (member.permissions.bitfield >= msg.guild.me.permissions.bitfield) throw '<:crossmark:508590460688924693>  ::  I cannot mute this user.';
		}

		reason = reason.length ? reason.join(this.usageDelim) : null;
		const muteRole = msg.guild.roles.get(msg.guild.settings.muteRole);
		if (!muteRole) throw '<:crossmark:508590460688924693>  ::  Whoops! The mute role has been deleted. Please reconfigure this server\'s mute role by using the `s.muterole` command.';
		if (muteRole.position >= msg.guild.me.roles.highest.position) throw `<:crossmark:508590460688924693>  ::  The mute role **${muteRole.name}** is higher than me, so I can't give ${user.tag} the mute role.`; // eslint-disable-line max-len
		if (member.roles.has(muteRole.id)) throw `<:crossmark:508590460688924693>  ::  ${user.tag} has been already muted!`;
		...
		msg.channel.send(`<:check:508590521342623764>  ::  **${user.tag}** (\`${user.id}\`) has been muted. ${reason ? `**Reason**: ${reason}` : ''}`);
