...
module.exports = class extends Command {
	...
	async run(msg, [member, ...reason]) {
		...
		if (muteRole.position >= msg.guild.me.roles.highest.position) throw `${this.client.constants.EMOTES.xmark}  ::  The mute role **${muteRole.name}** is higher than me, so I can't take from ${user.tag} the mute role.`; // eslint-disable-line max-len
		if (!member.roles.cache.has(muteRole.id)) throw `${this.client.constants.EMOTES.xmark}  ::  ${user.tag} wasn't muted already!`;
