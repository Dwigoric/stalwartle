...
module.exports = class extends Command {
	...
	async handleMessage(msg, options, result) {
		switch (options.sendAs) {
			case 'file': {
				if (msg.channel.attachable) return msg.channel.sendFile(Buffer.from(result), 'banlist.txt', `${this.client.constants.EMOTES.tick}  ::  Sent the ban list as a file.`);
			...
			case 'hastebin': {
				...
				if (options.url) return msg.sendMessage(`${this.client.constants.EMOTES.tick}  ::  Sent the ban list to hastebin: ${options.url}`);
