...
module.exports = class extends Command {
	...
	constructor(...args) {
		...
		this.createCustomResolver('channel', async (arg, possible, msg, [action]) => {
			...
			if (arg) {
				...
				if (!modlog.postable) throw `<:crossmark:508590460688924693>  ::  It seems that I cannot send messages in ${modlog}.`;
				return modlog;
			} else { throw '<:crossmark:508590460688924693>  ::  Please provide the modlog channel.'; }
		});
	}
	...
	async run(msg, [modlog]) {
		...
		return msg.send(`<:check:508590521342623764>  ::  Successfully updated the modlog channel for all moderation actions to ${modlog}.`);
	}
	...
	async reset(msg) {
		...
		return msg.send('<:check:508590521342623764>  ::  Successfully reset the modlog channel for all moderation actions.');
	}
	...
	async indivSet(msg, modlog, action) {
		...
		return msg.send(`<:check:508590521342623764>  ::  Successfully updated the modlog channel for member ${action}s to ${modlog}.`);
	}
