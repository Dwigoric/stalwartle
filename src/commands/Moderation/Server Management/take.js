...
module.exports = class extends Command {
    ...
    async messageRun(msg, [member, role]) {
        ...
        if (member.roles.highest.position >= msg.guild.me.roles.highest.position) throw `${this.container.constants.EMOTES.xmark}  ::  I cannot take ${role.name} from this user.`;
        if (!member.roles.cache.has(role.id)) throw `${this.container.constants.EMOTES.xmark}  ::  ${member} already doesn't have **${role.name}**! I mean, what's the point of taking something from someone they already don't have?`; // eslint-disable-line max-len
        await member.roles.remove(role, `Taken using ${this.container.client.user.username}'s Take Role feature`);
        return msg.send(`${this.container.constants.EMOTES.tick}  ::  Successfully taken the role **${role.name}** from ${member}.`);
    }
