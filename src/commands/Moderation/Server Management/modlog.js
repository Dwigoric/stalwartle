...
module.exports = class extends Command {
	...
	async run(msg, [user]) {
		...
		let list = this.client.gateways.modlogs.get(msg.guild.id, true).get('modlogs').sort((a, b) => parseInt(a.id) - parseInt(b.id));
	...
	async reset(msg) {
		...
		if (prompt.content === 'yes') {
			await this.client.gateways.modlogs.get(msg.guild.id, true).update('modlogs', []);
