...
module.exports = class extends SubCommandPluginCommand {
    ...
    async mute(msg, args) {
        ...
        if (!(await msg.guild.fetchAuditLogs()).entries.filter(entry => entry.action === 'MEMBER_UPDATE' && entry.changes.some(change => change.key === 'communication_disabled_until') && entry.executor.id === msg.author.id && entry.target.id === target.id && entry.changes[0].key === '$add' && entry.changes[0].new && entry.changes[0].new[0].id === msg.guild.settings.get('muteRole')).size) return reply(msg, `${this.container.constants.EMOTES.xmark}  ::  The server audit logs show that there are no instances that you gave the muterole to **${target.tag}**.`); // eslint-disable-line max-len
