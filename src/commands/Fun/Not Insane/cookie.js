...
module.exports = class extends Command {
    ...
    async messageRun(msg, [person]) {
        ...
        if (person.id === msg.author.id) throw `${this.container.constants.EMOTES.xmark}  ::  I know this command gives someone a cookie, but you can't give yourself a cookie! Don't be greedy 😿`;
    ...
    async lb(msg) {
        ...
        const message = await msg.channel.send(`${this.container.constants.EMOTES.loading}  ::  Loading leaderboard...`);
    ...
    async getList() {
        return Promise.all(await this.container.databases.default.getAll('users').then(usr => usr
