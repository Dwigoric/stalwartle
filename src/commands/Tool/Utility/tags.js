...
module.exports = class extends Command {
	...
	constructor(...args) {
		...
		this
			...
			.createCustomResolver('contents', (arg, possible, msg, [action]) => {
				...
				if (['create', 'edit'].includes(action) && !arg) throw '<:error:508595005481549846>  ::  Please provide the new contents of the tag.';
				return arg;
			});
	}
	...
	async run(msg, [userTag]) {
		...
		const noTagErr = `<:error:508595005481549846>  ::  Sorry! You have no tag named **${userTag}**.`;
	...
	async edit(msg, [tag, ...contents]) {
		if (!await this.client.providers.default.get('tags', msg.author.id)) await this.client.providers.default.create('tags', msg.author.id);
		const userTags = await this.client.providers.default.get('tags', msg.author.id);
		if (tag === 'id') throw `<:error:508595005481549846>  ::  Tag **${tag}** cannot be edited. Sorry!`;
		if (!userTags[tag]) throw `<:error:508595005481549846>  ::  Tag **${tag}** doesn't exist or isn't yours!`;
