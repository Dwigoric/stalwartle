...
module.exports = class extends Command {
	...
	async run(msg, [sar]) {
		...
		if (msg.member.roles.has(role.id)) {
			...
			return msg.send(`<:check:508594899117932544>  ::  **${escapeMarkdown(role.name)}** has been taken from **${escapeMarkdown(msg.member.displayName)}** via selfrole.`);
		} else {
			await msg.member.roles.add(role, `[Selfrole Add] Selfrole feature of ${this.client.user.username}`);
			return msg.send(`<:check:508594899117932544>  ::  **${escapeMarkdown(msg.member.displayName)}** has been given **${escapeMarkdown(role.name)}** via selfrole.`);
		}
	}
