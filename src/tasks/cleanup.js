	...
	async run() {
		...
		for (const guild of this.client.guilds.values()) {
			...
			for (const [id, member] of guild.members) {
				...
				if (await this.client.providers.get(this.client.options.gateways.users.provider).get('users', member.user.id).then(usr => usr.cookies)) continue;
				guildMembers++;
		...
		for (const user of this.client.users.values()) {
			...
			if (await this.client.providers.get(this.client.options.gateways.users.provider).get('users', user.id).then(usr => usr.cookies)) continue;
