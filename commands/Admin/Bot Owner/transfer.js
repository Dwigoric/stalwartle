...
module.exports = class extends Command {
	...
	async run(msg, [piece]) {
		...
		try {
			...
			return msg.send(msg.language.get('COMMAND_TRANSFER_SUCCESS', piece.type, piece.name));
		} catch (err) {
			this.client.emit('error', err.stack);
			return msg.send(msg.language.get('COMMAND_TRANSFER_FAILED', piece.type, piece.name));
		}
	}
