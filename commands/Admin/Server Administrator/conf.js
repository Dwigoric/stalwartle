...
module.exports = class extends Command {
	...
	show(msg, [key]) {
		...
		if (path.piece.type === 'Folder') {
			return msg.sendMessage(msg.language.get('COMMAND_CONF_SERVER', key ? `: ${key.split('.').map(toTitleCase).join('/')}` : '',
				codeBlock('asciidoc', msg.guild.settings.list(msg, path.piece))));
		}
		return msg.sendMessage(msg.language.get('COMMAND_CONF_GET', path.piece.path, msg.guild.settings.resolveString(msg, path.piece)));
	}

	async set(msg, [key, ...valueToSet]) {
		const { errors, updated } = await msg.guild.settings.update(key, valueToSet.join(' '), msg.guild, { avoidUnconfigurable: true, action: 'add' });
		...
		return msg.sendMessage(msg.language.get('COMMAND_CONF_UPDATED', key, msg.guild.settings.resolveString(msg, updated[0].piece)));
	}

	async remove(msg, [key, ...valueToRemove]) {
		const { errors, updated } = await msg.guild.settings.update(key, valueToRemove.join(' '), msg.guild, { avoidUnconfigurable: true, action: 'remove' });
		if (errors.length) return msg.sendMessage(errors[0]);
		if (!updated.length) return msg.sendMessage(msg.language.get('COMMAND_CONF_NOCHANGE', key));
		return msg.sendMessage(msg.language.get('COMMAND_CONF_UPDATED', key, msg.guild.settings.resolveString(msg, updated[0].piece)));
	}
	...
	async reset(msg, [key]) {
		const { errors, updated } = await msg.guild.settings.reset(key, msg.guild, true);
		if (errors.length) return msg.sendMessage(errors[0]);
		if (!updated.length) return msg.sendMessage(msg.language.get('COMMAND_CONF_NOCHANGE', key));
		return msg.sendMessage(msg.language.get('COMMAND_CONF_RESET', key, msg.guild.settings.resolveString(msg, updated[0].piece)));
	}
