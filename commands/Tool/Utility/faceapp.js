...
module.exports = class extends Command {
	...
	async run(msg, [filter]) {
		...
		if (!attachment || !attachment.height) throw `<:redTick:399433440975519754>  ::  Please upload an image together with the command.`;

		const { body: image } = await get(attachment.url).catch(() => {
			throw `<:redTick:399433440975519754>  ::  I could not download the file. Can you try again with another image?`;
		});
		...
		const faceappImage = await faceapp
			...
			.then(img => img)
			.catch(() => {
				throw `<:redTick:399433440975519754>  ::  Sorry! I couldn't find a face in the image.`;
			});
